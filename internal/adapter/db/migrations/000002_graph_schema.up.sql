...
CREATE TABLE graph.extracted_entity(
    ...
    memory_id UUID NOT NULL REFERENCES init.memories(id) ON DELETE CASCADE,
    name TEXT NOT NULL,
...
CREATE TABLE graph.memory_edges (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id UUID NOT NULL REFERENCES init.tenants(id) ON DELETE CASCADE,
    source_memory_id UUID NOT NULL REFERENCES init.memories(id) ON DELETE CASCADE,
    target_memory_id UUID NOT NULL REFERENCES init.memories(id) ON DELETE CASCADE,
    type TEXT NOT NULL,
    weight REAL DEFAULT 1.0,
